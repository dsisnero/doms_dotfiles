...
alias -g C='$(git log --oneline | ${FILTER_CMD} | head -1 | cut -d" " -f1)'  # Select commit from current branch
alias -g CALL='$(git log --decorate --branches --all --graph --oneline --all | ${FILTER_CMD} | head -1 | grep -o "[a-f0-9]\{7,\}" | head -1)'  # Select commit from all branches
